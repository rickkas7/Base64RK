...
void runTests() {
	...
	for(size_t testNum = 0; testNum < numTestEntries; testNum++) {
			...
			if (strcmp(encoded, testEntries[testNum].enc) == 0) {
				...
				if (success) {
					if (decodedLen == testEntries[testNum].dataLen) {
						...
						DEBUG_PRINT("test %zu decode length mismatch got %zu expected %zu\n", testNum, decodedLen, testEntries[testNum].dataLen);
					}
				}
				else {
					DEBUG_PRINT("test %zu decode returned false\n", testNum);
				}
				...
				DEBUG_PRINT("test %zu encoded data did not match\n", testNum);
				DEBUG_PRINT("got: %s\n", encoded);
				DEBUG_PRINT("exp: %s\n", testEntries[testNum].enc);
			}
		}
		else {
			DEBUG_PRINT("test %zu encode returned false (with termination)\n", testNum);
		}
		...
		if (success) {
			if (encodedLen == strlen(testEntries[testNum].enc)) {
				if (strncmp(encoded, testEntries[testNum].enc, encodedLen) == 0) {
					...
					DEBUG_PRINT("test %zu encoded data did not match (no termination)\n", testNum);
				}
			}
			else {
				DEBUG_PRINT("test %zu encoded length did not match (no termination)\n", testNum);
			}
		}
		else {
			DEBUG_PRINT("test %zu encode returned false (no termination)\n", testNum);
		}
		...
		if (encodedLen > 1) {
			...
			success = Base64::encode(testEntries[testNum].data, testEntries[testNum].dataLen, encoded, encodedLen, false);
			if (success) {
				DEBUG_PRINT("test %zu returned success but should have failed with buffer too small", testNum);
			}
		...
		if (encodedStr.length() == (int)strlen(testEntries[testNum].enc)) {
			if (strcmp(encodedStr, testEntries[testNum].enc) == 0) {
				...
				DEBUG_PRINT("test %zu encoded data did not match (string)\n", testNum);
			}
		}
		else {
			DEBUG_PRINT("test %zu encoded length did not match (string)\n", testNum);
		}
	}
